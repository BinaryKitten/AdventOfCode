...
function isGameValid(string $game): bool
{
    $stages = explode(';', $game);
    foreach ($stages as $stage) {
        ...
        if ($picks['red'] > 12 || $picks['green'] > 13 || $picks['blue'] > 14) {
            return false;
        }
    }
...
function validGameIdSum(string $fileName): int
{
    $games = file(__DIR__.'/'.$fileName);

    $validGameIds = [];
    foreach ($games as $game) {
        [$gameId, $game] = explode(': ', $game);

        if (isGameValid($game)) {
            $validGameIds[] = str_replace('Game ', '', $gameId);
        }
    }
...
echo 'Example Result: '.validGameIdSum('example.txt')."\n";
echo 'Result: '.validGameIdSum('input.txt')."\n\n";