...
function groupDepthsBySlidingGroups(array $depths, int $amount): array
{
    $groups = [];
    for ($i = 0, $l = count($depths); $i < $l; $i++) {
        $groups[] = array_sum(array_slice($depths, $i, $amount));
    }
    return $groups;
}

function countIncreasesInDepth(string $file): int
{
    $increaseCount = 0;
    $lastDepth = null;
    $depths = loadFileValues($file);
    foreach (groupDepthsBySlidingGroups($depths, 3) as $newDepth) {
        if ($lastDepth === null) {
            $lastDepth = $newDepth;
            continue;
        }
        $increaseCount += (int)($lastDepth < $newDepth);
        $lastDepth = $newDepth;
    }
...
echo 'Example Result: ' . countIncreasesInDepth('example.txt') . "\n";
echo 'Result: ' . countIncreasesInDepth('input.txt') . "\n\n";
