 ...
 * be able to track the keys if you can boost its signal strength high enough; there's a little meter that indicates
...
function countIncreasesInDepth(string $file): int
{
    $increaseCount = 0;
    $lastDepth = null;
    foreach (loadFileValues($file) as $newDepth) {
        if ($lastDepth === null) {
            $lastDepth = $newDepth;
            continue;
        }
