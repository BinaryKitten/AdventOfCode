 ...
 * the prototype fabric, the design plans, everything! Nobody on the team can even seem to remember important
...
$checksum = generate_checksum($test['data']);
printf('%d === %d: %s',
    $test['checksum'],
    $checksum,
    $test['checksum'] === $checksum ? 'Pass' : 'Fail'
);
echo "\n";

function generate_checksum(array $data): int
{
    $twos = 0;
    $threes = 0;
    foreach ($data as $boxId) {
        $counts = get_counts($boxId);
        $twos += (int)in_array(2, $counts, true);
        $threes += (int)in_array(3, $counts, true);
    }
    return $twos * $threes;
}

function get_counts(string $boxId): array
{
    $letter_counts = array_count_values(str_split($boxId));
    $filtered = array_filter($letter_counts, function ($count) {
        return $count === 2 || $count === 3;
    });

    $keys = array_keys(array_flip($filtered));
    sort($keys);
    return $keys;
}
