...
function countBags(string $bagToFind, array $bags): int
{
    $bagCount = 1;
    foreach ($bags[$bagToFind] as $childBag => $childBagAmount) {
        $bagCount += countBags($childBag, $bags) * $childBagAmount;
    }

    return $bagCount;
}

$testBags = parseRules(__DIR__ . '/test2.txt');
$testBagCount = countBags('shiny gold', $testBags) - 1;

$bags = parseRules(__DIR__ . '/input.txt');
$bagCount = countBags('shiny gold', $bags) - 1;

echo 'TEST: Total bags required: ' . $testBagCount . ' bags' . PHP_EOL;
echo 'Total bags required: ' . $bagCount . ' bags' . PHP_EOL;
