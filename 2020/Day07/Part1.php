...
function findBag(string $bagToFind, array $bags): array
{
    $bagsFoundIn = [];
    foreach ($bags as $bag => $bagContents) {
        if (array_key_exists(key: $bagToFind, array: $bagContents)) {
            $bagsFoundIn[] = [$bag];
            $bagsFoundIn[] = findBag($bag, $bags);
        }
    }

    return array_unique(array_merge(...$bagsFoundIn));
}

$bags = parseRules(__DIR__ . '/test.txt');
$goldBagsFoundIn = findBag('shiny gold', $bags);
echo 'TEST: Gold Bags found through ' . count($goldBagsFoundIn) . ' bags' . PHP_EOL;

$bags = parseRules(__DIR__ . '/input.txt');
$goldBagsFoundIn = findBag('shiny gold', $bags);
echo 'Gold Bags found through ' . count($goldBagsFoundIn) . ' bags' . PHP_EOL;
