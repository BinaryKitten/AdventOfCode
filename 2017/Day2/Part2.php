...
while ($row = fgetcsv($file, 0, "\t")) {
    ...
    foreach ($row as $idx => $this_number) {
        foreach ($row as $idx2 => $row_number) {
            if ($this_number > $row_number) {
                continue;
            }
            if ($idx === $idx2) {
                continue;
            ...
            if ($row_number % $this_number !== 0) {
                continue;
            }

            $row_values[] = $row_number / $this_number;
            break 2;
        }
    }
}

echo 'Result: ' . array_sum($row_values) . "\n\n";
