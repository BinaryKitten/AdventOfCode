...
$nouns = $verbs = range(0, 99);
$targetValue = 19690720;

foreach($nouns as $noun) {
    foreach($verbs as $verb) {
        $result = intCodeCompute('data/input.txt', [1 => $noun, 2 => $verb]);
        if ($result === $targetValue) {
            printf('Noun %d, Verb %d === %d'."\n", $noun, $verb, $targetValue);
            break 2;
        }
    }
}

printf('the Result for 2019-12-02 Part 2: %d' . "\n", (100 * $noun + $verb));
